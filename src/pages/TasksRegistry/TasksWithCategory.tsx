    ...
    <>
      {categories.map((category: ICategory) => {
        const filteredTasks = filterTasks[registryType].filter(
          (task: ITask) => task.Select.value === category.select.value,
        );

        if (!filteredTasks.length) {
          return (
            <li className="self-start mt-5 self-center text-3xl border-2 border-r-0 border-l-0 border-t-0 border-b-blue">
              Для категории <OrangeText>{category.select.value}</OrangeText> нет
              {registryType === "task"
                ? " Заметок"
        ...
        return (
          <li className="flex flex-col items-center border-2 border-r-0 border-l-0 border-t-0 border-b-blue w-4/5 min-h-12 flex items-center py-5">
            <OrangeText className="text-3xl mb-5">{category.select.value}</OrangeText>
